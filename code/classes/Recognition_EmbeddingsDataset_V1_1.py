...
class EmbeddingsDataset:
    ...
    def encode(self, unitStep):
        ...
        embedded = self.network.predict(self.prepareImage(unitStep))[0] 
    ...
    def predictEmbeddings(self):
        ...
        x, y = self.dataset.getDataset(allDataset, interleaved=False, shuffled=False)
        predicted = self.network.predict(x)[0]
    ...
    def getMeanEncoded(self, dataIndex):
        ...
        encoded = self.network.predict(x)[0]
        return(encoded, y)
