...
class Autoencoder:
    #constructor
    def __init__(self, unitSize, alpha=1.2, beta=1.0, lamda=0.0001, learningRate=0.001):
    ...
    def combinedLoss(self, x):
        ...
        decoded = x[2]
        c_mean = x[3]        
        loss = combined_loss(y_true, y_pred, decoded, c_mean, self.alpha, self.beta, self.lamda)
        return(loss)
