...
def combined_loss(y_true, y_pred, decoded, c_mean, alpha, beta, lamda, lossType=0):
    ...
    num_positives = tf.math.reduce_sum(mask_positives)
    DAE_loss[0] = tf.math.truediv(sum_norm_positives, num_positives)
    DAE_loss[1] = tf.math.reduce_mean(norm_squared)
    
    #***********************************************************************
    # Combined Loss
    #***********************************************************************    
    loss = tf.math.add(triplet_loss, lamda * DAE_loss[lossType])
    return loss  